...
class HeuristicTest(testClasses.TestCase):
    ...
    def checkHeuristic(self, heuristic, problem, state, solutionCost):
        ...
        if not h0 <= solutionCost:
            return False, 'Heuristic failed admissibility test' + ' solution is '+ str(solutionCost) + ' your result ' + str(h0)
        ...
        for child, action, stepCost in problem.expand(state):
            ...
            if h0 - h1 > stepCost: return False, 'Heuristic failed consistency test '+ str(h0) + '-' + str(h1) + ">" + str(stepCost)
...
###
# Added by Chris Ewin March 2020
###
class DeceptiveTestpi2(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(DeceptiveTestpi2, self).__init__(question, testDict)
        self.layoutText = testDict['layout']
        self.layoutName = testDict['layoutName']
    ...
    def solution(self, searchAgents):
        ...
        gameState = pacman.GameState()
        gameState.initialize(lay, 0)
        path = searchAgents.DeceptiveSearchAgentpi2().registerInitialState(gameState)
        return path
...
class DeceptiveTestpi3(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(DeceptiveTestpi3, self).__init__(question, testDict)
        self.layoutText = testDict['layout']
        self.layoutName = testDict['layoutName']
    ...
    def solution(self, searchAgents):
        ...
        gameState = pacman.GameState()
        gameState.initialize(lay, 0)
        path = searchAgents.DeceptiveSearchAgentpi3().registerInitialState(gameState)
        return path
